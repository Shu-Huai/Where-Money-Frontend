...
async function applyAiOutAsset(outAssetId: number | null, outAssetName: string | null): Promise<void> {
    if (outAssetId === null) return;
    assetId.value = outAssetId;
    try {
        const response: AssetGetAssetResponse = await getAssetApi({id: outAssetId});
        assetName.value = response.asset.assetName;
        assetBalance.value = response.asset.balance;
        assetSvg.value = response.asset.svg;
    } catch (_err) {
        if (outAssetName !== null) {
            assetName.value = outAssetName;
        }
    }
}
...
async function parseAiBill(): Promise<void> {
    ...
    try {
        ...
        await applyAiOutAsset(data.outAssetId, data.outAssetName);
        closeAiModal();
    } catch (error: any) {
