        ...
        <n-tooltip :placement="activeConfig.placement" trigger="hover">
            <template #trigger>
                <div class="layout-checkbox__shadow relative w-56px h-48px bg-white rounded-4px overflow-hidden">
                    <div :class="activeConfig.menuClass" class="absolute-lt bg-[#273352]"></div>
                    <div :class="activeConfig.mainClass" class="absolute-rb bg-[#f0f2f5]"></div>
                </div>
            </template>
            <span>{{ label }}</span>
        </n-tooltip>
...
type LayoutConfig = Record<ThemeLayoutMode,
    {
